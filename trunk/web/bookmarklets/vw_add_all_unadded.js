...
var recruit_url = 'http://apps.facebook.com/vampiresgame/recruit.php';
var add_url = 'http://apps.facebook.com/vampiresgame/status_invite.php?from=';
var mw_url = 'http://apps.facebook.com/vampiresgame/stats.php?user=';
...
var content = document.getElementById('app25287267406_content');
...
function state_change() {
	...
	if (xmlHTTP.status == 200) {	// 200 = OK
	    ...
	    if (m = /You have added (.+) to your clan/.exec(s)) {
		friends_added++;
		msg('Added ' + mwlink(m[1]) + ' AKA ' + fblink() + '.');
//	    } else if (m = /You are already part of (.+)\'s mafia/.exec(s)) {
//		msg('Already added ' + mwlink(m[1]) + ' AKA ' + fblink() + '.');
	    } else if (/app25287267406_content/.test(s) || /window\.location\.replace/.test(s)) {
		// If game loaded, but we didn't get the message, we probably have a user who doesn't play the game.
...
function begin() {
	...
	msg('Found ' + i + ' friends not yet in your clan');
